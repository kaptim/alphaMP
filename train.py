...
# pulls information from the train.yaml in the config folder
@hydra.main(version_base="1.3", config_path=str(here() / "config"), config_name="train")
def main(cfg):
    ...
    log.info(
        f"Test results ({cfg.test_runs} runs): {np.mean(scores):.4f} +- {np.std(scores):.4f}"
    )
    results = {
