...
def custom_f1(preds, targets):
    return f1_score(
        preds, targets, average="macro", task="multiclass", num_classes=preds.shape[-1]
    )
...
def custom_ogbg(y_pred, y_true, evaluator, metric):
    ...
    y_pred = y_pred.cpu().numpy()
    y_true = y_true.cpu().numpy()
    score = evaluator.eval({"y_pred": y_pred, "y_true": y_true})[metric]
    return torch.tensor(score)
...
def setup_metric_function(metric, metric_name, dataset_name):
    if "ogbg" in metric_name:
        ...
        metric = partial(metric, evaluator=evaluator, metric=metric_name.split("-")[-1])
    return metric
