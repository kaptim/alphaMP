...
class Mpnn_pl(pl.LightningModule):
    def __init__(self, cfg):
        super().__init__()

        self.model = Mpnn(**OmegaConf.to_container(cfg.model, resolve=True))
        self.cfg = cfg
        ...
        self.metric = setup_metric_function(
            self.metric, cfg.dataset.metric, cfg.dataset.name
        )
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log(
            "train_loss", loss, on_step=False, on_epoch=True, batch_size=len(batch.y)
        )
        return loss
    ...
    def val_test_step(self, batch, batch_idx, step="val"):
        if "edge" in self.cfg.model.head:
            ...
            stats = {
                ...
                if self.cfg.dataset.metric in key
            }
            mean_stats = {
                f"{step}_{key}": sum(value) / len(value) for key, value in stats.items()
            }
            self.log_dict(
                mean_stats, on_step=False, on_epoch=True, batch_size=batch.num_graphs
            )
            self.validation_step_outputs.append(stats)
            return loss
        loss, y_pred, y_true = self.compute_loss(batch, return_y=True)
        self.log(
            f"{step}_loss", loss, on_step=False, on_epoch=True, batch_size=len(batch.y)
        )
        if self.cfg.dataset.name == "ogbg-code2":
            mat = []
    ...
    def on_validation_epoch_end(self):
        if "edge" in self.cfg.model.head:
            score = torch.cat(
                [
                    torch.tensor(out[self.cfg.dataset.metric])
