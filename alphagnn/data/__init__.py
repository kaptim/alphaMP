...
def get_dataset(cfg, transform=None, test_transform=None, pre_transform=None):
    ...
    if cfg.dataset.name == "zinc":
        ...
        train_idx, val_idx = clip_graph_size(
            dataset, split_idx, getattr(cfg.dataset, "max_num_nodes", None)
        )
        dataset = LightningDataset(
...
def clip_graph_size(dataset, split_idx, max_num_nodes=None):
    ...
    train_mask = (
        np.array([dataset[i].num_nodes for i in split_idx["train"]]) <= max_num_nodes
    )
    val_mask = (
        np.array([dataset[i].num_nodes for i in split_idx["valid"]]) <= max_num_nodes
    )
    print(
        f"Ratio of graphs with size <= {max_num_nodes}: {np.sum(train_mask) / len(split_idx['train'])}"
    )
